...
import 'wearable_connection_screen.dart';  // Updated import
...
class _SorenessTrackerScreenState extends State<SorenessTrackerScreen>
    ...
    setState(() {
      if (_showingFront) {
        if (_selectedFront.contains(id)) {
          ...
          _showSorenessPopup(id); // Show popup for this muscle
        }
      } else {
  ...
  void _showSorenessPopup(String muscleId) {
                      ...
                      Flexible(
                        child: Row(
                          ...
                          children: List.generate(5, (index) {
                            final level = index + 1;
                            final isSelected = _sorenessLevels[muscleId] == level;
              ...
              builder: (context, child) {
                return CustomPaint(
                  ...
                  painter: _HighlightPainter(
                    ...
                    sorenessLevels: _sorenessLevels,
                  ),
                );
              },
            ),
        ...
        child: ScaleTransition(
          ...
          child: GestureDetector(
            onTap: () {
              ...
              // Updated navigation to go through WearableConnectionScreen
              Navigator.push(
                ctx,
                PageRouteBuilder(
                  pageBuilder: (_, __, ___) => WearableConnectionScreen(
...
// Highlight painter with soreness-based opacity
class _HighlightPainter extends CustomPainter {
  ...
  final Map<String, int> sorenessLevels;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final id in selected) {
      final path = paths[id];
      if (path != null) {
        // Get soreness level (default to 5 if not set)
        final sorenessLevel = sorenessLevels[id] ?? 5;

        // Calculate opacity based on soreness level (1 = 0.2, 10 = 0.8)
        final baseOpacity = 0.15 + (sorenessLevel * 0.065);
        final fillOpacity = baseOpacity + (pulseValue * 0.1);
        final glowOpacity = baseOpacity * 0.6;

        // Dynamic red color based on soreness
        final paint = Paint()
          ..color = Colors.red.withOpacity(fillOpacity)
  ...
  bool shouldRepaint(covariant _HighlightPainter old) =>
      !setEquals(old.selected, selected) ||
          ...
          !mapEquals(old.sorenessLevels, sorenessLevels);
}
